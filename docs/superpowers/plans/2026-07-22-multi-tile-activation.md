...
**Task 1~4, 6은 하드웨어·카메라·인형 없이 완결된다.** Task 5는 아두이노만, Task 7~9는 카메라가 필요하다.
...
**주의:** 이 태스크는 자동 테스트가 없다. 아두이노 시리얼 모니터로 수동 검증한다.
...
Arduino IDE에서 이 스케치를 열고 보드를 선택한 뒤 업로드한다.
컴파일 오류가 없어야 한다.

- [ ] **Step 3: 시리얼 모니터로 수동 검증**
...
| §7.4 리셋 | Task 5, Task 9 |
| §7.6 인덱싱 통일, 단일 문자 제거 | Task 5 |
| §7.7 펌웨어 변경 범위 | Task 5 |
| §8 프로파일·CLI·`camera_yaw_deg` | Task 6, 7, 9 |
| §9 로깅 컬럼 | Task 9 |
| §10.1 단위 테스트 | Task 1, 2, 3, 4, 6 |
| §10.2 하드웨어 검증, 서보 이동 시간 | Task 5 |
