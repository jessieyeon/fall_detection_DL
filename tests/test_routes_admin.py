...
def test_deleting_resident_keeps_camera(client):
    ...
    linked = next(c for c in cams if c["resident_id"] is not None)

    assert client.delete(f"/api/admin/residents/{linked['resident_id']}").status_code == 200

    after = {c["id"]: c for c in client.get("/api/admin/cameras").json()}
    assert linked["id"] in after
    assert after[linked["id"]]["resident_id"] is None
...
def test_scan_finds_only_unregistered_devices(client):
    ...
    cameras.registry.announce("daon-cam-302")        # 이미 등록된 기기
...
def test_register_rejects_duplicate_device(client):
    r = client.post("/api/admin/cameras",
                    json={"device_key": "daon-cam-302", "name": "중복",
                          "location": "복도"})
...
def test_camera_resident_can_be_cleared_and_set(client):
    cams = client.get("/api/admin/cameras").json()
    cam = next(c for c in cams if c["resident_id"] is not None)
    original = cam["resident_id"]
...
def test_dispatch_for_unit_camera_names_the_resident(client):
    cams = client.get("/api/admin/cameras").json()
    cam = next(c for c in cams if c["location"] == "세대 내부")
