...
프로파일 세 가지:
  human  평가용 기본 동작점(thr .05 / pers 3). 감지율 15/17 을 지키는 대신
         오경보가 자체 클립 기준 1.83건/영상이다.
  demo   전시 시연용. 임계값을 0.30 으로 올리고 persistence 를 4 로 늘려
         가만히 서 있을 때 타일이 튀는 것을 억제한다. 약한 낙상은 놓칠 수
         있지만, 관람객 앞에서 '아무도 안 넘어졌는데 펴지는' 장면이 나오는
         쪽이 시연에서는 더 나쁘다. 넘어지는 시늉은 점수가 0.7 이상으로
         올라가므로(리허설 로그 기준) 이 임계값에서도 잡힌다.
  doll   인형 시연용.
"""
